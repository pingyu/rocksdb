...
Status FilePrefetchBuffer::Prefetch(const IOOptions& opts,
  ...
  Status s = Read(opts, reader, for_compaction, read_len, chunk_len,
...
Status FilePrefetchBuffer::PrefetchAsyncInternal(
    ...
    size_t length, size_t readahead_size, bool for_compaction,
  ...
  }
  {
    ...
    ReadAsync(opts, reader, for_compaction, read_len2, chunk_len2,
              rounddown_start2, second)
        .PermitUncheckedError();
  }

  if (read_len1 > 0) {
    s = Read(opts, reader, for_compaction, read_len1, chunk_len1,
      ...
      if (for_compaction) {
        ...
        if (async_io_) {
          ...
          s = PrefetchAsyncInternal(opts, reader, offset, n,
                                    readahead_size_ / 2, for_compaction,
...
Status FilePrefetchBuffer::PrefetchAsync(const IOOptions& opts,
  ...
  s = ReadAsync(opts, reader, for_compaction, read_len, chunk_len,
